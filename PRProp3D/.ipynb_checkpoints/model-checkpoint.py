    ...
    for j in range(nbatches):
      ...
      for i in range(batchsize): # total number of z steps (niter) if one batch
          ...
          if jt%skip==0:
            ...
            #ampxz=appender_ind('ampxzt',outputs,ampxz,amp)
            #dnxz=appender_ind('dnxzt',outputs,dnxz,dnfull[i])
    ...
    if jt%skip == 0:
      ...
      if prdata.backpropagate:
        ...
        for i in range(niter):
            ampbft=ampbft*cp.conj(h)
            ampb=cp.fft.ifft2(ampbft)*windowxy
        #ampxyt = appender_ind('ampxyt',outputs,ampxyt,ampb)
        if 'ampxyt' in outputs: appender_ind('ampxyt',outputs,ampxyt,ampb)
      else:
        #ampxyt = appender_ind('ampxyt',outputs,ampxyt,amp)
